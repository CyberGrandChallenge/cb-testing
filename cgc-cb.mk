...
RELEASE_DEBUG_PATH = $(BUILD_DIR)/$(PATCHED_DIR)/bin/$(EXE)
...
# Polls that the CB author intends to release the resulting network traffic during CQE
	if [ -d poller/for-release ]; then cb-test --timeout 15 --port $(TCP_PORT) --cb $(filter-out %_patched, $(notdir $(wildcard bin/$(EXE)*))) --xml_dir poller/for-release/ --directory $(BIN_DIR) --log $(BUILD_DIR)/$(RELEASE_EXE).for-release.txt --pcap pcap/$(RELEASE_EXE)_poll.pcap ; fi
...
.PHONY: all clean clean-test patched prep release remake test $(BINS)
